...
class AccountInvoice(models.Model):
    ...
    @api.multi
    def account_invoice_not_paid_auto_send_mail_item_real(
            self,
            mail_template_id,
            author_id
    ):
            ...
            if not item.date_invoice_not_paid_send_mail:
                if item.state in ['open', 'out_invoice']:
                    if item.amount_total > 0 and item.residual > 0:
                        item_pm = item.payment_mode_id
                        if item_pm.payment_method_id.code != 'sepa_direct_debit':
                            template_id = int(
                                self.env['ir.config_parameter'].sudo().get_param(
                                    'account_invoice_not_paid_template_id'
                                )
                            )
                            item.account_invoice_not_paid_auto_send_mail_item_real(
                                template_id
                            )
